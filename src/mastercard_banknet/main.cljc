(ns mastercard-banknet.main
  "Kotodama WASM entrypoint for the Mastercard_banknet clean-room actor (L4) — Clojure port.
  ...
  facts (`mastercard_banknet.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mastercard_banknet")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mastercard_banknet.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mastercard_banknet-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mastercard_banknet-compat/ready)
