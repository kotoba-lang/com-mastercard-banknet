(ns mastercard-banknet.main-test
  "Contract + behavioral test for the mastercard_banknet-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mastercard_banknet-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mastercard-banknet.main-test)]
